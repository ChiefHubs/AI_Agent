...
const Home = () => {
  ...
  const getQueries = async () => {
    ...
    await getAllQueries()
      .then((res) => {
        console.log("queries ", res.data.chats.Sales);
  ...
  return (
    <>
      ...
      {isMenuOpen ? (
        <div
          className={`overflow-hidden w-[30%] flex flex-col absolute left-0 top-0 h-full min-h-screen transition-all ease-in-out duration-300`}
        >
          <Sidebar
            queries={queries}
